...
void ST7735_DrawCharS(int16_t x, int16_t y, char c, int16_t textColor, int16_t bgColor, uint8_t size, uint8_t pt_size, uint8_t b_size, const uint8_t *font_type){
  ...
  if((x >= _width)            || // Clip right
     ...
     ((x + pt_size * size - 1) < 0) || // Clip left
     ((y + (b_size+1) * size - 1) < 0))   // Clip top
    return;
...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor, uint16_t bg_color, uint8_t size){
  ...
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6*size, y*10, *pt, textColor, bg_color, size, 5, 7, Font);
    pt++;
    x = x+1;
...
void ST7735_OutUDec(uint32_t n){
  ...
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1,  5, 7, Font);
  }
}
...
void ST7735_OutChar(char ch){
  ...
  ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_YELLOW,ST7735_BLACK, 1,  5, 7, Font);
  StX++;
  if(StX>20){
    StX = 20;
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1,  5, 7, Font);
